...
> 처음에는 리뷰가 없는 식당도 출력해야 한다고 생각해서 INNER JOIN 대신 LEFT JOIN을 사용하고, 리뷰가 없는 식당의 평균 점수에 나타나는 NULL 값을 처리하기 위해 IFNULL 함수도 함께 사용했는데, 그렇게 작성하니 오답으로 나왔다. 그래서 INNER JOIN으로 바꾸고 IFNULL 함수도 제거했더니(왜냐하면 INNER JOIN을 사용하면 NULL 값이 생기지 않기 때문) 정답이 나왔다. 다음부터는 NULL 처리에 대한 명시적인 설명이 없으면 그냥 INNER JOIN으로 푸는 게 좋겠다.
